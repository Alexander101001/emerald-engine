...
export class SecureOrchestrator {
  ...
  getTelegramBotId() {
    const standalone = this.get('TELEGRAM_BOT_ID');
    if (standalone && !standalone.startsWith('[INSERT')) return standalone;
    const cfg = this.getTelegramConfig();
    return cfg ? cfg.bot_id || null : null;
  }
...
async function initOrchestrator() {
  try {
    ...
    if (telegramCfg) {
      ...
      if (telegramCfg.bot_id) process.env.TELEGRAM_BOT_ID = telegramCfg.bot_id;
    }
    const botId = orchestrator.getTelegramBotId();
    if (botId) process.env.TELEGRAM_BOT_ID = botId;
