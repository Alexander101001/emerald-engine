...
export class AutonomousLifecycle {
  ...
  async deploy(vaultPassphrase) {
    ...
    if (telegramCfg && telegramCfg.bot_token && !telegramCfg.bot_token.startsWith('[INSERT')) {
      await telegram.initialize(telegramCfg.bot_token, telegramCfg.user_id, telegramCfg.channel_id);
      try {
        await telegram.sendToChannel('System Awakened: Autonomous Mode');
      } catch {}
    }
