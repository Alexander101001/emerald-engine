...
import uptimeWatchdog from './uptime-watchdog.js';
...
import viral from '../modules/viralGrowthEngine.js';
...
export class AutonomousLifecycle {
  ...
  async deploy(vaultPassphrase) {
    ...
    try {
      ...
      if (telegramCfg && telegramCfg.bot_token && !telegramCfg.bot_token.startsWith('[INSERT')) {
        telegram.reset();
        const botId = orchestrator.getTelegramBotId();
        const initResult = await telegram.initialize(telegramCfg.bot_token, telegramCfg.user_id, telegramCfg.channel_id, botId);
        if (initResult.active) {
          ...
          try { await telegram.sendToOwner('System Online'); } catch (e) {
            logger.warn(`lifecycle: admin handshake failed — ${e.message}`);
          }
        }
      }
    } catch (e) {
      logger.warn(`lifecycle: bridge init failed — ${e.message} — neural core continues`);
    }

    logger.info(`lifecycle: aegis=${aegisStatus.status} neural=${neuralStatus.state} vault=${this._vaultLoaded}`);

    try {
      uptimeWatchdog.activate();
      uptimeWatchdog.registerCheck('neural-core', () => {
        const c = neuralCore.getConsciousness();
        return c.state === 'awake' && c.activeNeurons > 0;
      }, 30000);
      uptimeWatchdog.registerCheck('telegram-bridge', () => telegram.getStatus().active || telegram.getStatus().retryActive, 30000);
      uptimeWatchdog.registerCheck('aegis', () => aegis.getStatus().active, 60000);
    } catch (e) {
      logger.warn(`lifecycle: uptime watchdog init failed — ${e.message}`);
    }
    ...
    this._scheduleGrowthLoop();
    this._scheduleViralGrowth();
  ...
  _scheduleGrowthLoop() {
    const id = setInterval(async () => {
      try {
        logger.info('growth-loop: scanning for top SaaS trends...');
        const trends = await scrapeTrends(null);
        logger.info(`growth-loop: collected ${trends.length} SaaS-ranked trends`);

        const highValue = trends.filter(t => t.saasScore >= 8).slice(0, 5);
        if (highValue.length === 0) {
          logger.info('growth-loop: no high-value trends found this cycle');
          return;
        }

        for (const trend of highValue) {
          try {
            const product = {
              productName: trend.title,
              tagline: (trend.description || trend.title).slice(0, 120),
              category: trend.category || 'productivity',
              keyFeatures: ['SaaS-ready', 'Trend-optimized', 'Stripe integrated', 'SEO optimized'],
              monetization: 'freemium',
            };
            const repos = await createRepo(product);
            if (repos) {
              logger.info(`growth-loop: spawned repo for "${trend.title}" — ${repos.url || repos.repoName}`);
              try { await telegram.sendToChannel(`Growth Loop spawned: ${trend.title} — ${repos.url || repos.repoName}`); } catch {}
              product.url = repos.url || `https://${slugify(trend.title)}.vercel.app`;
              try {
                const campaign = await viral.executeCampaign(product);
                const report = viral.generateReport(campaign);
                logger.info(`viral: campaign for "${trend.title}"\n${report}`);
                try { await telegram.sendToChannel(`Viral Campaign: ${trend.title}\n${report.slice(0, 2000)}`); } catch {}
              } catch (ve) {
                logger.warn(`viral: campaign failed for "${trend.title}" — ${ve.message}`);
              }
            }
          } catch (e) {
            logger.warn(`growth-loop: failed to spawn "${trend.title}" — ${e.message}`);
          }
        }
      } catch (e) {
        logger.warn(`growth-loop: cycle failed — ${e.message}`);
        this._failures.push({ type: 'growth_loop', error: e.message, ts: Date.now() });
      }
    }, 3600000);
    this._intervalIds.push(id);

    logger.info('lifecycle: growth loop scheduled (3600s interval)');
  }

  _scheduleViralGrowth() {
    const id = setInterval(async () => {
      try {
        const trends = await scrapeTrends(null);
        const topTrends = trends.filter(t => t.saasScore >= 6).slice(0, 2);
        ...
        for (const trend of topTrends) {
          const product = {
            ...
            tagline: (trend.description || trend.title).slice(0, 120),
            category: trend.category || 'productivity',
            url: `https://${slugify(trend.title)}.vercel.app`,
            keyFeatures: ['SaaS-ready', 'Trend-optimized', 'Stripe integrated'],
            monetization: 'freemium',
          };
          const campaign = await viral.executeCampaign(product);
          const report = viral.generateReport(campaign);
          logger.info(`viral: campaign report for "${product.productName}"\n${report}`);
          try {
            await telegram.sendToChannel(`Viral Campaign Complete\n${report}`);
          } catch {}
        }
      } catch (e) {
        logger.warn(`viral: growth cycle failed — ${e.message}`);
        this._failures.push({ type: 'viral_growth', error: e.message, ts: Date.now() });
      }
    }, 21600000);
    this._intervalIds.push(id);

    logger.info('lifecycle: viral growth engine scheduled (21600s interval)');
  }
  ...
  getLifecycleStatus() {
    return {
      ...
      watchdog: watchdog.getStatus(),
      uptimeWatchdog: uptimeWatchdog.getStatus(),
      viral: viral.getSummary(),
      telegram: telegram.getStatus(),
    };
  }
